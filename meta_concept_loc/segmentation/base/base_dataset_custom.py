...
class BaseDataSetCustom(Dataset):
    ...
    def _val_augmentation(self, image, label):
        if self.crop_size:
            ...
            label = [Image.fromarray(l).resize((w, h), resample=Image.NEAREST) for l in label]
            label = [np.asarray(l, dtype=np.int32) for l in label]
            ...
            label = [l[start_h:end_h, start_w:end_w] for l in label]
    ...
    def _augmentation(self, image, label):
        ...
        if self.base_size:
            ...
            image = cv2.resize(image, (w, h), interpolation=cv2.INTER_LINEAR)
            label = [cv2.resize(l, (w, h), interpolation=cv2.INTER_NEAREST) for l in label]
        ...
        if self.rotate:
            ...
            label = [cv2.warpAffine(l, rot_matrix, (w, h), flags=cv2.INTER_NEAREST) for l in label]#,  borderMode=cv2.BORDER_REFLECT)
        ...
        if self.crop_size:
            ...
            if pad_h > 0 or pad_w > 0:
                ...
                label = [cv2.copyMakeBorder(l, value=0, **pad_kwargs) for l in label]
        ...
        if self.flip:
            if random.random() > 0.5:
                ...
                label = [np.fliplr(l).copy() for l in label]
