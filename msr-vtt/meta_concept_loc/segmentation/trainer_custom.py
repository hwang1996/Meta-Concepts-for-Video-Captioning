    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(tbar):
            ...
            if self.config['arch']['type'][:3] == 'PSP':
                ...
                loss = self.loss(output[0], target.float())
                loss += self.loss(output[1], target.float()) * 0.4
                output = output[0]
            else:
                ...
                shape = self.train_loader.dataset.crop_size
            ...
            if len(train_visual) < 8:
                target_np = target[:, 0].data.cpu().numpy()
                output_np = output[:, 0].data.cpu().numpy()
