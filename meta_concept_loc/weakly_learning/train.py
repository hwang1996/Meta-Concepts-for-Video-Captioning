...
def main():
    ...
    # Read word map
    word_map = train_dataset.wordtoix
    ...
    for step, (imgs, caps, caplens) in enumerate(train_loader):
        ...
        for i in range(batch_size):
            for j in range(img_len):
                ...
                w = j%int(img_len/2)
                imgs_new[i, ih*h:ih*(h+1), iw*w:iw*(w+1), :] = imgs[i, j]
        # import pdb; pdb.set_trace()
        scores, caps_sorted, decode_lengths, alphas, sort_ind, loss_tri = decoder(imgs_new, caps, caplens)
        ...
        for step, (imgs, caps, caplens, allcaps) in enumerate(val_loader):
            ...
            if step % print_freq == 0:
                print('Validation: [{0}/{1}]\t'
                      ...
                      'Top-5 Accuracy {top5.val:.3f} ({top5.avg:.3f})\t'.format(step, len(val_loader), batch_time=batch_time,
            ...
            for j in range(allcaps.shape[0]):
                ...
                img_captions = list(map(lambda c: [w for w in c if w not in {0, 1}], img_caps))  # remove <start> and pads
